...
# Check that src/main/resources/conf/deepzoom.cfg has workingDir Property is setted to absolute path to webapp directory (this is needed to be changed if you want to use a data diretory outside of tomcat webapp
...
For running the service on production environment it's highly recommended to store tiles outside the deployed servlet container. For that you have to create directories outside tomcats webapps directory and contextes for tomcat to access these directories. 

Its also recommended to generate an apache vhost to access the service via proxy or rewrite. 

# Get with: git clone https://github.com/hbz/DeepZoomService.git
# Create web accessible temp and tilesCache directory outside tomcat's webapp directory [1].
...
fn1. For ubuntu compare with implementation of /usr/share/tomcat7-example via /etc/tomcat7/Catalina/example.xml
